...
//      Telegram Channel : @XuVixC
//      Github Channel : @XuVix

import { connect } from "cloudflare:sockets";

// How to generate your own UUID:sfsdfg
// https://www.uuidgenerator.net/
let userID = "7fbc0b07-9f5c-433f-bbac-4ef76121e2ff";


const ipfrag = "www.speedtest.net"
const ipcf = [
            ...
            if (!upgradeHeader || upgradeHeader !== "websocket") {
                ...
                switch (url.pathname) {
                    ...
                    case `/sub`: 
                        ...
                        const vlessConfigs = client === "singbox" ? await env.abol.get("singbox-sub") : await env.abol.get("xray-sub");
                    ...
                    */       
                    case `/frag/${userID}`:
                        ...
                        const configs = JSON.parse(await env.abol.get("fragConfigs"));                        
                    ...
                    case `/frag`:
                        ...
                        const configs2 = JSON.parse(await env.abol.get("fragConfigs"));                        
                        const fragConfig2 = configs2.find(conf => conf.address === ipfrag)?.fragConf;
                        return new Response(`${JSON.stringify(fragConfig2, null, 4)}`, {
                            status: 200,
                            headers: {
                                "Content-Type": "text/plain;charset=utf-8",
                            },
                    ...
                    case `/bpbpanel`:
                        ...
                        const hostValue = await env.abol.get("host");
                        ...
                        if (hostValue !== host) await env.abol.put("host", host);
                            
                        if (request.method === "POST" || await env.abol.get("fragConfigs") === null || hostValue !== host) {
...
const getVLESSConfig = async (env, userID) => {
    let vlessWsTls = "";
    const cleanIPs = await env.abol.get("cleanIPs");
    const hostName = await env.abol.get("host");
    ...
    Addresses.forEach((addr) => {
		counter++;
        vlessWsTls += `vless://${userID}@${addr}:${TLS}?encryption=none&security=tls&type=ws&host=${randomUpperCase(
            hostName
        )}&sni=${randomUpperCase(
            hostName
        )}&fp=randomized&alpn=http/1.1&path=%2F${lable}%3Fed%3D2048#${lable}%20-%20${counter}%20-%20TLS\n`;
    });
	Addresses.forEach((addr) => {
		counter++;
        vlessWsTls += `vless://${userID}@${addr}:${NoneTLS}?encryption=none&security=none&type=ws&host=${randomUpperCase(
            hostName
        )}&sni=${randomUpperCase(
            hostName
        )}&fp=randomized&alpn=http/1.1&path=%2F${lable}%3Fed%3D2048#${lable}%20-%20${counter}\n`;
    });

    const singboxSub = btoa(vlessWsTls);
    const xraySub = btoa(vlessWsTls.replaceAll("http/1.1", "h2,http/1.1"));
    await env.abol.put("xray-sub", xraySub);
    await env.abol.put("singbox-sub", singboxSub);
};
...
const getFragVLESSConfig = async (env, userID) => {
    ...
    await env.abol.put("fragConfigs", JSON.stringify(Configs));
};
...
const renderPage = async (env, uuid) => {
	...
	<body>
		<h1 style="text-align: center; color: #2980b9">abol Panel <span style="font-size: smaller;">2.1</span> 💦</h1>
		<div class="form-container">
			...
			<div class="table-container">
				<table id="normal-configs-table">
					...
					<tr>
						...
						<td>
                            <button onclick="copyToClipboard('https://${host}/sub', false)">
                                Copy 
					...
					<tr>
						...
						<td>
                            <button onclick="copyToClipboard('https://${host}/sub?app=singbox', false)">
                                Copy 
                                <span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
						</td>
            ...
            <div class="footer">
                ...
                <a class="link" href="https://github.com/bia-pain-bache/abol-Worker-Panel" target="_blank">
...
const updateDataset = async (env, host, remoteDNS, localDNS, lengthMin, lengthMax, intervalMin, intervalMax, cleanIPs) => {
    ...
    for (const [key, value] of Object.entries(initData)) {
        await env.abol.put(key, value);
    }
};

const getDataset = async (env) => {
    const host = await env.abol.get("host");
    const remoteDNS = await env.abol.get("remoteDNS");
    const localDNS = await env.abol.get("localDNS");
    const lengthMin = await env.abol.get("lengthMin");
    const lengthMax = await env.abol.get("lengthMax");
    const intervalMin = await env.abol.get("intervalMin");
    const intervalMax = await env.abol.get("intervalMax");
    const cleanIPs = await env.abol.get("cleanIPs");
    const fragConfigs = JSON.parse(await env.abol.get("fragConfigs"));
